...
trait ClientSender: Send + Sync + 'static {
    ...
    fn write_text_file(
        ...
        Box<
            dyn Future<Output = Result<agent_client_protocol::schema::WriteTextFileResponse, Error>>
                + Send
                + '_,
        >,
    >;
...
fn skill_command_items(skill: &SkillMetadata, rest: &str) -> Vec<UserInput> {
    let mut items = vec![UserInput::Skill {
        ...
        path: skill.path.clone().to_path_buf(),
    }];
...
mod tests {
    ...
    async fn test_load_exposes_user_skills_as_available_commands() -> anyhow::Result<()> {
        let (_session_id, client, _, message_tx, _handle) =
            setup_with_skills(vec![test_skill("design-skill")]).await?;
    ...
    async fn setup_with_skills(
        ...
        actor.skills = Arc::new(std::sync::Mutex::new(skills));
    ...
    impl ClientSender for StubClient {
        ...
        fn write_text_file(
            ...
            Box<
                dyn Future<
                        Output = Result<
                            agent_client_protocol::schema::WriteTextFileResponse,
                            Error,
                        >,
                    > + Send
                    + '_,
        ...
        > {
            Box::pin(async move {
                ...
                Ok(agent_client_protocol::schema::WriteTextFileResponse::new())
            })
        }
    }
